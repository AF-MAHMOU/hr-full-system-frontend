...
import { getNotifications } from './api/orgStructureApi';
...
function OrganizationStructureContent() {
  ...
  const [departmentNotificationCount, setDepartmentNotificationCount] = useState(0);
  const [changeRequestNotificationCount, setChangeRequestNotificationCount] = useState(0);
  ...
  useEffect(() => {
    if (user?.userid) {
      const fetchNotificationCounts = async () => {
        try {
          const [deptResponse, crResponse] = await Promise.all([
            getNotifications({ type: 'department', limit: 100 }),
            getNotifications({ type: 'change_request', limit: 100 }),
          ]);

          const deptUnread = (deptResponse.data || []).filter((n: any) => !n.read).length;
          const crUnread = (crResponse.data || []).filter((n: any) => !n.read).length;

          setDepartmentNotificationCount(deptUnread);
          setChangeRequestNotificationCount(crUnread);
        } catch (err) {
          ...
          <div className={styles.tabs}>
            ...
            >
              ...
              {departmentNotificationCount > 0 && (
                <span className={styles.tabBadge}>{departmentNotificationCount > 99 ? '99+' : departmentNotificationCount}</span>
              )}
            </button>
            <button
            ...
            >
              ...
              {changeRequestNotificationCount > 0 && (
                <span className={styles.tabBadge}>{changeRequestNotificationCount > 99 ? '99+' : changeRequestNotificationCount}</span>
              )}
            </button>
          </div>
