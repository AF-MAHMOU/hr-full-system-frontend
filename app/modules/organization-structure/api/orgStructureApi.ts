...
export async function getNotifications(params?: {
  type?: 'department' | 'change_request' | string;
  limit?: number;
}): Promise<{
  success: boolean;
  message: string;
  ...
  const response = await apiClient.get(
    `${API_ENDPOINTS.ORGANIZATION_STRUCTURE}/notifications`,
    { params }
  );
  return response.data;
}
...
export async function markNotificationAsRead(notificationId: string): Promise<{
  ...
  const response = await apiClient.put(
    `${API_ENDPOINTS.ORGANIZATION_STRUCTURE}/notifications/${notificationId}/read`
  );
  return response.data;
}
...
export async function markAllNotificationsAsRead(): Promise<{
  ...
  const response = await apiClient.put(
    `${API_ENDPOINTS.ORGANIZATION_STRUCTURE}/notifications/read-all`
  );
  return response.data;
}
