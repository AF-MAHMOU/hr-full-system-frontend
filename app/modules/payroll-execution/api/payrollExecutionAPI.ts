...
const TRACKING_URL = API_ENDPOINTS.PAYROLL_TRACKING;
...
export const payrollrunApi = {
  ...
  /**
   * REQ-PY-1: Get specific payslip for employee
   */
  getEmployeePayslip: async (employeeId: string, payslipId: string): Promise<PaySlip> => {
    const response = await apiClient.get<PaySlip>(
      `${TRACKING_URL}/employee/${employeeId}/payslips/${payslipId}`
    );
    return response.data;
  },
  ...
  getEmployeePayslips: async (
    ...
    const url = `${TRACKING_URL}/employee/${employeeId}/payslips${params.toString() ? `?${params.toString()}` : ''}`;
    const response = await apiClient.get<PaySlip[]>(url);
    return response.data;
  },
  ...
  getEmployeeBaseSalary: async (employeeId: string): Promise<any> => {
    const response = await apiClient.get(
      `${TRACKING_URL}/employee/${employeeId}/base-salary`
    );
    return response.data;
  },
  ...
  getSalaryHistory: async (employeeId: string): Promise<any[]> => {
    const response = await apiClient.get(
      `${TRACKING_URL}/employee/${employeeId}/salary-history`
    );
    return response.data;
  },
   ...
   */
  downloadPayslipPDF: async (employeeId: string, payslipId: string): Promise<Blob> => {
    const response = await apiClient.get(
      `${TRACKING_URL}/employee/${employeeId}/payslips/${payslipId}/download`,
      { responseType: 'blob' }
    );
    return response.data;
  },
